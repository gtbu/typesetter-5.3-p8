...
namespace gp{
	...
	class Page extends Base{
		...
		public function __construct($title, $type){
			...
			// canonical href: validate/escape before inserting into head
			$url = \gp\tool::AbsoluteUrl($title);
			if( is_string($url) ){
				$safeUrl = htmlspecialchars($url, ENT_QUOTES | ENT_HTML5, 'UTF-8');
				$this->head .= '<link rel="canonical" href="'.$safeUrl.'" />'."\n";
			} else {
				// fallback: don't include canonical if not a string
			}
		}
		...
		public function GetSiteLabel(){
			global $config;
			echo htmlspecialchars($config['title'], ENT_QUOTES | ENT_HTML5, 'UTF-8');
		}
		public function GetSiteLabelLink(){
			global $config;
			// Pass an escaped label into Link(); if Link() already escapes it, this is conservative
			echo \gp\tool::Link('', htmlspecialchars($config['title'], ENT_QUOTES | ENT_HTML5, 'UTF-8'));
		}
		public function GetPageLabel(){
			echo htmlspecialchars($this->label, ENT_QUOTES | ENT_HTML5, 'UTF-8');
		}
		...
		public function GetGpxContent(){
			...
			if( isset($this->meta_data['file_number']) ){
				// ensure file_number is an integer to avoid class injection
				$fileNum = intval($this->meta_data['file_number']);
				$class = 'filenum-'.$fileNum;
			}
			...
			// build class attribute and escape
			$classAttr = trim($class . ' cf');
			$classAttr = htmlspecialchars($classAttr, ENT_QUOTES | ENT_HTML5, 'UTF-8');

			echo '<div id="gpx_content" class="'.$classAttr.'">';

			// contentBuffer may contain HTML generated by the rendering pipeline (Sections::Render / plugins).
			// Those functions are responsible for sanitization. We echo the buffer directly here.
